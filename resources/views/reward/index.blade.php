        ...
        <main class="px-6 py-8 lg:px-10">
            <header class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-col items-end">
                    ...
                    <p class="text-4xl font-black text-emerald-500">{{ number_format($totalPoin, 0, ',', '.') }} <span class="text-xl text-slate-500">pts</span></p>
                    <a href="{{ route('poin.index') }}" class="text-sm text-emerald-600 hover:underline mt-1">Lihat riwayat poin →</a>
                </div>
            </header>
                    ...
                    @forelse($rewards as $reward)
                        ...
                        <article class="bg-[#f0f9f3] rounded-xl overflow-hidden border border-slate-800 flex flex-col group hover:shadow-lg transition">
                            ...
                            <div class="h-44 bg-[#7db993] flex items-center justify-center relative border-b border-slate-800">
                                <div class="text-7xl drop-shadow-md">{{ $icon }}</div>
                                @if($reward->stok < 10)
                                    <span class="absolute top-3 right-3 bg-red-500 text-white text-[10px] font-black uppercase px-2 py-1 rounded-md border border-slate-800 shadow-sm">Sisa {{ $reward->stok }}</span>
                                @endif
                            </div>
                            
                            <!-- Bottom Half -->
                            <div class="p-5 flex flex-col flex-1 items-center text-center">
                                <h3 class="text-xl font-black text-slate-900">{{ $reward->nama }}</h3>
                                <p class="mt-1 text-base font-medium text-slate-700">{{ number_format($reward->poin_diperlukan, 0, ',', '.') }} pts</p>
