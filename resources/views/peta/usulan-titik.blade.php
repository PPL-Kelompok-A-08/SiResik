...
<body class="min-h-screen bg-slate-100 text-slate-900">
            ...
            <section class="mt-10 rounded-[2.5rem] border border-slate-200 bg-white p-8 shadow-sm shadow-slate-200/50">
                <div class="mx-auto max-w-3xl">
                    <div class="text-center">
                        ...
                        <p class="mt-2 text-slate-500">Bantu kami meningkatkan layanan dengan lokasi titik penjemputan strategis di wilayah Bojongsoang.</p>
                    </div>
                    ...
                    <form class="mt-8 space-y-6" action="{{ route('peta.usulan-titik.store') }}" method="POST">
                        ...
                        <div>
                            ...
                            <p id="koordinat-terpilled" class="mt-2 text-sm text-slate-500">Ketuk peta Bojongsoang untuk menentukan koordinat.</p>
                            <input type="hidden" id="latitude" name="latitude" value="{{ old('latitude') }}">
                            <input type="hidden" id="longitude" name="longitude" value="{{ old('longitude') }}">
                        </div>

                        <div>
                            <label class="mb-2 block text-xs font-black uppercase tracking-[0.18em] text-slate-400">Detail Alamat / Landmark</label>
                            <textarea id="alamat_detail" name="alamat_detail" rows="3" required
                                placeholder="Contoh: Samping gerbang komplek, dekat counter pulsa"
                        ...
                        <div>
                            <label class="mb-2 block text-xs font-black uppercase tracking-[0.18em] text-slate-400">Jenis Layanan</label>
                        ...
                        <div>
                            <label class="mb-2 block text-xs font-black uppercase tracking-[0.18em] text-slate-400">Deskripsi / Alasan Usulan</label>
                            <textarea id="deskripsi_alasan" name="deskripsi_alasan" rows="4" required
                                placeholder="Jelaskan alasan kenapa lokasi di wilayah ini membutuhkan titik penjemputan..."
    ...
    <script>
    (function () {
        const bojongsoang = [-6.9745, 107.6322];
        const map = L.map('usulan-map', { scrollWheelZoom: true }).setView(bojongsoang, 14);
        ...
        const selectedText = document.getElementById('koordinat-terpilled');
        let marker = null;
