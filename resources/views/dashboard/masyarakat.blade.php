...
<head>
    ...
    <meta name="description"
        content="Dashboard masyarakat SiResik – pantau setoran, poin, peta layanan, dan riwayat transaksi Anda.">
    ...
    <style>
        * {
        ...
        .mas-logout-nav:hover {
            background: rgba(239, 68, 68, .15) !important;
            color: #fca5a5 !important;
        }

        .mas-logout-nav .nav-icon {
        ...
        #mas-nav-scroll::-webkit-scrollbar {
            width: 4px;
        }

        #mas-nav-scroll::-webkit-scrollbar-thumb {
...
<body class="text-slate-900">
    ...
    <div id="mas-mobile-bar"
        ...
        <button onclick="toggleMasSidebar()"
    ...
    <div id="mas-overlay" onclick="toggleMasSidebar()"
                ...
                <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 flex flex-col">
                    ...
                    <div class="flex-1 divide-y divide-slate-50">
                        ...
                        @forelse ($trackingRequests as $index => $item)
                            ...
                            @endphp
                            <div
                                class="grid grid-cols-[auto,1fr,auto] items-center gap-2 px-5 py-3 hover:bg-slate-50/60 transition">
                                ...
                                <span
                                    class="inline-flex rounded px-2 py-0.5 text-[10px] font-black whitespace-nowrap {{ $statusMeta['class'] }}">
                                    {{ $statusMeta['label'] }}
                                </span>
                            </div>
                        @empty
                ...
                <div class="rounded-[2.5rem] bg-white p-6 shadow-xl shadow-slate-200/60 ring-1 ring-slate-200">
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p class="text-sm text-slate-500">Daftar lengkap transaksi dan partisipasi kebersihan Anda.
                            </p>
                        </div>
                        <button
                            class="rounded-2xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700">Unduh
                            Laporan</button>
                    </div>
                            ...
                            <tbody>
                                @foreach ($permintaan as $item)
                                    ...
                                    <tr class="border-t">
                                        ...
                                        <td class="px-4 py-4">{{ optional($item->created_at)->translatedFormat('d M Y') }}
                                        </td>
                                        <td class="px-4 py-4">{{ $item->alamat ?? '-' }}</td>
                                        <td class="px-4 py-4 text-slate-400">{{ $kategoriText ?: '-' }}</td>
                                        <td class="px-4 py-4 text-right text-emerald-600 font-black">
                                            +{{ number_format($points) }} Pts</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <footer class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 px-6 py-3">
                <div class="flex flex-wrap items-center gap-x-8 gap-y-2 text-xs text-slate-500">
                    ...
                    <span>
                        ...
                        <span class="ml-1 text-emerald-600 font-semibold">{{ number_format($stats['total'] ?? 0) }}
                            Setoran</span>
                    </span>
    ...
    <div id="detailModal"
        ...
        <div
            class="relative w-full max-w-[640px] transform scale-95 bg-white rounded-[2.5rem] shadow-2xl transition-all duration-300 overflow-hidden flex flex-col">
            <!-- Modal Header -->
            <div class="bg-[#00c48c] px-10 py-8 text-white relative">
                <!-- Close Button -->
                <button onclick="closeDetailModal()"
                    class="absolute top-6 right-6 flex h-10 w-10 items-center justify-center rounded-full bg-white/20 hover:bg-white/30 text-white font-black text-xl transition-all cursor-pointer">
                    &times;
                </button>
                ...
                <div class="flex flex-wrap gap-3 mt-4">
                    <span id="modalStatusBadge"
                        class="rounded-2xl bg-white/20 px-4 py-1.5 text-sm font-black text-white">SELESAI</span>
                    <span id="modalDateBadge"
                        class="rounded-2xl bg-white/20 px-4 py-1.5 text-sm font-black text-white">15 MAR 2024</span>
                </div>
            </div>
        </div>
    </div>
